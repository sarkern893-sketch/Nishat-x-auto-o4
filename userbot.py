        ...
        for repeat_index in range(count):
            ...
            try:
                if group_settings.get("ai_enabled") and (event.message.text or event.message.message):
                    ...
                    await send_message(group, event.message, edited)
                else:
                    try:
                        ...
                        # Feature 8 — plain forward blocked → try a direct copy instead
                        # of giving up immediately.
                        copied = await copy_via_user_client(source_chat, event.id, group)
                        if not copied:
                            raise forward_error
        ...
        for group in groups:
            try:
                try:
                    ...
                    copied = await copy_via_user_client(source_chat, message_id, group)
                    if not copied:
                        raise forward_error
            except Exception as error:
