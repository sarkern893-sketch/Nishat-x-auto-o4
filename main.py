...
ALL_MENU_BUTTONS = {
    ...
    "➕ Source যোগ", "➕ User যোগ", "➕ Watch Channel যোগ", "➖ Destination বাদ", "➖ Filter বাদ",
    "➖ Source বাদ", "➖ Watch Channel বাদ", "⬅️ AI সেটিংসে ফিরুন", "⬅️ ফিরে যান",
    "🎨 AI Style", "🎨 Chat Style", "🎨 Post Format Style", "🎭 AI পরিচয় সেটিংস", "🎯 Border",
    "🎯 Select Group", "🏠 Destination", "👋 Welcome সেটিংস", "👑 Owner-এর নাম",
    "👑 অ্যাডমিন ব্যবস্থাপনা", "👤 @username", "👥 Admin তালিকা", "👥 Group AI সেটিংস",
    "👥 Group List", "👥 User List", "💬 Live Chat সেটিংস", "📄 Master নির্দেশনা (Text/File)",
    "📅 Schedule", "📇 Personal তথ্য", "📊 Status", "📊 পরিসংখ্যান", "📋 Filter তালিকা",
    "📋 Watch Channel তালিকা", "📏 Post Length", "📚 Private Knowledge", "📝 Common Message",
    "📝 History", "📝 অটো পোস্ট", "📝 টেমপ্লেট", "📞 ফোন", "📡 Source", "📡 চ্যানেল সেটিংস",
    "📢 Channel → Group", "📥 Buffer Status", "📩 Contact Line", "📩 ইউজার মেসেজিং",
    "📩 এখন পাঠান", "📰 Header", "🔁 Context ON/OFF", "🔁 Forward সেটিংস",
    "🔁 Similarity Filter ON/OFF", "🔐 Permission সেট", "🔗 t.me লিংক", "🔘 Bullet ON/OFF",
    "🔢 Forward Count", "🔢 Max Posts/Slot", "🔤 Emoji Heading ON/OFF", "🔤 Word Filter",
    "🔴 AI Editing বন্ধ", "🔴 Campaign বন্ধ", "🔴 C→G বন্ধ", "🔴 File বন্ধ",
    "🔴 Format Style বন্ধ", "🔴 Image বন্ধ", "🔴 Live Chat বন্ধ", "🔴 Multi Watch বন্ধ",
    "🔴 Welcome বন্ধ", "🔴 বন্ধ করুন", "🔻 Footer", "🖼️ মিডিয়া ফিল্টার", "🗑️ Admin সরান",
    "🗑️ User সরান", "🚫 অতিরিক্ত Filter/নিষেধ", "🛡️ প্রাইভেসি ফিল্টার", "🟢 AI Editing চালু",
    "🟢 Campaign চালু", "🟢 C→G চালু", "🟢 File চালু", "🟢 Format Style চালু",
    "🟢 Image চালু", "🟢 Live Chat চালু", "🟢 Multi Watch চালু", "🟢 Welcome চালু",
    "🟢 চালু করুন", "🤖 AI Editing", "🤖 AI সেটিংস", "🤖 AI-র নাম", "🧠 Chat Prompt",
    "🧠 Custom Prompt", "🧭 মাল্টি-চ্যানেল ওয়াচ",
}
...
async def _handle_impl(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Bug fix — যদি কোনো text-input state চালু থাকা অবস্থায় (যেমন "User ID দিন"
    # জিজ্ঞেস করার পর) Admin কোনো মেনু বাটনে চাপে (যেমন "⬅️ ফিরে যান"), তাহলে
    # আগে সেই বাটনের টেক্সটটাই ভুল করে ID/username হিসেবে সংরক্ষিত হয়ে যেত —
    # যেমন "⬅️ ফিরে যান" নিজেই একটা "user" হয়ে যাচ্ছিল, পরে message পাঠানোর
    # সময় Telegram সেটাকে খুঁজে না পেয়ে crash/error notification পাঠাত।
    # এখন কোনো পরিচিত মেনু বাটনে চাপলে, চালু থাকা state আগে বাতিল হয়ে যায়,
    # যাতে বাটনের টেক্সট ডেটা হিসেবে সেভ না হয়ে স্বাভাবিক নেভিগেশন হিসেবে কাজ করে।
    if uid in user_state and t in ALL_MENU_BUTTONS:
        user_state.pop(uid, None)
    ...
    # ── Feature 4 — Multi-Channel Watch + Smart Scheduling ──
        ...
        await update.message.reply_text(
            ...
            "⚠️ জরুরি শর্ত: এখানে যোগ করা প্রতিটা Channel-এ আপনার Personal Account "
            "(যেটা দিয়ে বট Login করা) আগে থেকেই Member/Joined থাকতে হবে — ঠিক "
            "📡 Source-এর মতোই। Join করা না থাকলে সেই Channel থেকে কোনো Post "
            "Buffer-এ জমা হবে না, আর 📥 Buffer Status সবসময় 0 দেখাবে।",
